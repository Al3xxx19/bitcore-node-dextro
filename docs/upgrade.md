...
`bitcore-node-dextro@2.1.1` to `bitcore-node-dextro@3.0.0`
...
```json
{
  ...
  "servicesConfig": {
    "bitcoind": {
      "spawn": {
        "datadir": "/home/<username>/.bitcoin",
        "exec": "/home/<username>/bitcore-node-dextro/bin/bitcoind"
      }
    }
  }
}
...
**Important**: Once changes have been made you'll also need to add the `reindex=1` option **only for the first startup** to regenerate the indexes. Once this is complete you should be able to remove the `bitcore-node-dextro.db` directory with the old indexes.
