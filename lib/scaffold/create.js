...
var BASE_PACKAGE = {
  ...
  readme: 'README.md',
  dependencies: {
    'bitcore-lib-dinero': 'dinerocoin/bitcore-lib-dinero',
    'bitcore-node-dinero': 'dinerocoin/bitcore-node-dinero'
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/bitcore-node-dinero.json', configJSON);
      if (!isGlobal) {
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the bitcore-node-dinero directory and configuration
